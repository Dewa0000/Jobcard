    ...
    useEffect(() => {
        const fetchJob = async () => {
            try {
                ...
                setJob(data);//try catch only handles runtime error not parsetime error
            } catch (error) {
    ...
    return (
        <>
          <section className="container m-auto py-6 px-6">
            <Link
              to="/jobs" // Assuming you want to link to the homepage (adjust as needed)
              ...
              <div className="grid grid-cols-1 md:grid-cols-70/30 w-full gap-6">
                ...
                <aside>
                  ...
                  <div className="bg-white p-6 rounded-lg shadow-md mt-6">
                    ...
                    <Link
                      to = {`/edit-job/${job.id}`} // Assuming edit job route (adjust as needed)
...
const jobLoader = async ({ params }) => {
    const res = await fetch(`/api/jobs/${params.id}`);//It is a feature of react router not react
    const data = await res.json();
    return data;
}
